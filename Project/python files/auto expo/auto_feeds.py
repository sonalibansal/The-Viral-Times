...
def get_feed():
    ...
    cur = con.cursor()
    sql = "SELECT page_id,page_name FROM Pages_fan"
    cur.execute(sql)
    rows = cur.fetchall()
    sql_create_table = "CREATE TABLE IF NOT EXISTS Posts (page_id bigint unsigned, post_id varchar(100) NOT NULL PRIMARY KEY,created_time timestamp,message text,post_num bigint)"
